...
* ### 1. Producer-Consumer Simulation (Assignment 1)
* **The Context:** The screenshot demonstrates the system processing a simulated batch of 10 records (`Record-1` to `Record-10`).
* **What to look for:** You will notice the logs from the **Producer** and **Consumer** are interleaved (appearing in alternating sequences). This confirms that both threads are active simultaneously and correctly synchronizing via the blocking queue without data loss or race conditions.

### 2. Data Analysis Output (Assignment 2)
* **The Context:** These results were generated by processing the `data/sales_data.csv` file, which contains a subset of the Superstore dataset.
* **Verification:**
    * **Furniture Sales ($2000.33):** Validates the summation logic using `filter` and `reduce`.
    * **West Region Profit ($23.9):** Validates the averaging logic, proving the system correctly handled the floating-point math and row counting.
